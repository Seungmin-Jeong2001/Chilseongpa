...
# -------------------------------------------------------------------
# 4. 로컬 파일 생성 (apply 완료 후 자동 저장)
# -------------------------------------------------------------------
# GCP SA 키를 infra/terraform/gcp-sa-key.json으로 자동 저장
resource "local_file" "gcp_sa_key" {
  content         = module.gcp.db_proxy_sa_key
  filename        = "${path.module}/gcp-sa-key.json"
  file_permission = "0600"
}

# -------------------------------------------------------------------
# 5. GCP 모듈 (Primary 인프라)
